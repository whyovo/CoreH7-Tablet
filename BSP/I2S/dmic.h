 ...
 *
 * 使用示例:
    void DMIC_DataReadyCallback(int32_t *pData, uint32_t size, uint8_t
 isHalfBuffer)
    {
        // pData: 指向就绪数据的指针
        // size: 数据长度（采样点数）
 ...
 */
void DMIC_DataReadyCallback(int32_t *pData, uint32_t size,
                            uint8_t isHalfBuffer);
 ...
 * @note   在DMA中断中调用，应快速处理或复制数据
 */
typedef void (*DMIC_DataReadyCallback_t)(int32_t *pData, uint32_t size,
                                         uint8_t isHalfBuffer);
