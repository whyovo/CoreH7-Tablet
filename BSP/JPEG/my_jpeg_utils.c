/**
  ...
  ******************************************************************************
  ...
  ==============================================================================
    ...
    [..]For Decoding:
      - First, function "my_JPEG_InitColorTables" should be called to initialize the YCbCr to RGB color
      ...
      - As soon as the JPEG HW peripheral finished parsing the header of the JPEG input file,
        ...
        User can then call the utility function "my_JPEG_GetDecodeColorConvertFunc " with these
    ...
    [..]For Encoding:
      - First, function "my_JPEG_InitColorTables" should be called to initialize the YCbCr/RGB color
        conversion tables. This function needs to be called only one time at the beginning of the
        program whatever the number of jpeg files to be encoded or decoded.
    
      - First Use the utility function "my_JPEG_GetEncodeColorConvertFunc" with the input image information
...
// 添加编码用的CLAMP宏，防止YCbCr值溢出
#define CLAMP_ENCODE(x) (((x) < 0) ? 0 : (((x) > 255) ? 255 : (x)))
#endif
...
(*(pOutAddr + JPEG_ConvertorParams.Y_MCU_LUT[offset]))  = CLAMP_ENCODE(ycomp);
(*(pOutAddr + JPEG_ConvertorParams.Cb_MCU_LUT[offset])) = CLAMP_ENCODE(cbcomp);
(*(pOutAddr + JPEG_ConvertorParams.Cr_MCU_LUT[offset])) = CLAMP_ENCODE(crcomp);
...
#endif  
        ycomp  = (int32_t)(*(RED_Y_LUT + red)) + (int32_t)(*(GREEN_Y_LUT + green)) + (int32_t)(*(BLUE_Y_LUT + blue));
        (*(pOutAddr + JPEG_ConvertorParams.Y_MCU_LUT[offset + 1]))  = CLAMP_ENCODE(ycomp);
        ...
        (*(pOutAddr + JPEG_ConvertorParams.Y_MCU_LUT[offset + JPEG_ConvertorParams.H_factor]))  = CLAMP_ENCODE(ycomp);
...
#endif
        ycomp  = (int32_t)(*(RED_Y_LUT + red)) + (int32_t)(*(GREEN_Y_LUT + green)) + (int32_t)(*(BLUE_Y_LUT + blue));
        (*(pOutAddr + JPEG_ConvertorParams.Y_MCU_LUT[offset + JPEG_ConvertorParams.H_factor + 1]))  = CLAMP_ENCODE(ycomp);
  ...
  {        
    ...
    {  
      ...
      for(j=0; j < colones; j++)
      {
        ...
        my_kBlocks[i][j]=color_k;
        
(*(pOutAddr + JPEG_ConvertorParams.Y_MCU_LUT[offset]))  = CLAMP_ENCODE(red * 255 / color_k);
(*(pOutAddr + JPEG_ConvertorParams.Cb_MCU_LUT[offset])) = CLAMP_ENCODE(green * 255 / color_k);
(*(pOutAddr + JPEG_ConvertorParams.Cr_MCU_LUT[offset])) = CLAMP_ENCODE(blue * 255 / color_k);
    ...
    JPEG_Set_K_Blocks(pOutAddr, my_kBlocks, JPEG_ConvertorParams.ChromaSubsampling);
    pOutAddr +=  JPEG_ConvertorParams.BlockSize;
  }
...
/**
  ...
  */
HAL_StatusTypeDef my_JPEG_GetEncodeColorConvertFunc(JPEG_ConfTypeDef *pJpegInfo, JPEG_RGBToYCbCr_Convert_Function *pFunction, uint32_t *ImageNbMCUs)
{
  uint32_t hMCU, vMCU;
...
/**
  ...
  * @retval HAL status : HAL_OK or HAL_ERROR.
  */
HAL_StatusTypeDef my_JPEG_GetDecodeColorConvertFunc (JPEG_ConfTypeDef *pJpegInfo, JPEG_YCbCrToRGB_Convert_Function *pFunction, uint32_t *ImageNbMCUs)
{
  uint32_t hMCU, vMCU;
...
/**
  ...
  * @retval None
  */
void my_JPEG_InitColorTables(void)
{
