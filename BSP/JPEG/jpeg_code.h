 ...
 * JPEG_OUTPUT_DATA_BUFFER   -> JPEG解码输出缓冲区(1MB)
 * JPEG_ENCODE_OUTPUT_BUFFER -> JPEG编码输出缓冲区(1MB)
 ...
 * @note  大小: 1MB (0x200000)
 * @note  位于LCD显存之后，用于存储解码后的YCbCr数据
 * @note  在DMA2D中将YCbCr转换为RGB后输出到显存
 */
#define JPEG_OUTPUT_DATA_BUFFER                                                \
  (RGB_LCD_MemoryAdd + RGB_LCD_Width * RGB_LCD_Height * RGB_BytesPerPixel_0)
