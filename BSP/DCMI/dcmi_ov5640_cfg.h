/**
 ******************************************************************************
 * @file    dcmi_ov5640_cfg.h
 * @author  菜菜why(B站:菜菜whyy)
 * @brief   OV5640 摄像头参数配置头文件
 ******************************************************************************
 * @attention
 *
 * 包含 OV5640 的初始化寄存器序列、亮度/对比度/特效配置表以及自动对焦固件
 *
 ******************************************************************************
 */
...
#define CAM_MODE_SMALL_STD 0  // 小屏标准 (小于440x330, 43fps)
#define CAM_MODE_SMALL_HIGH 1 // 小屏高帧 (小于440x330, 60fps，可能有稳定性问题！)
#define CAM_MODE_LARGE_STD 2  // 大屏标准 (小于880x495, 30fps)
#define CAM_MODE_LARGE_HIGH 3 // 大屏高帧 (小于880x495, 36fps)

#ifdef OV5640_USE_HIGH_FPS

    #ifndef OV5640_WORK_MODE

        /* 1. 最高优先级：用户强制指定使用 SPI 屏幕分辨率 */
        #if defined(OV5640_USE_SPI_CAM) && defined(LCD_SPI_ENABLE)
        #define OV5640_WORK_MODE CAM_MODE_SMALL_HIGH

        /* 2. 其次匹配 RGB 屏幕 (默认策略) */
        #elif defined(LCD_RGB_ENABLE)
        #define OV5640_WORK_MODE CAM_MODE_LARGE_HIGH
    ...
    /* 1. 最高优先级：用户强制指定使用 SPI 屏幕分辨率 */
    #if defined(OV5640_USE_SPI_CAM) && defined(LCD_SPI_ENABLE)
    #define OV5640_WORK_MODE CAM_MODE_SMALL_STD

    /* 2. 其次匹配 RGB 屏幕 (默认策略) */
    #elif defined(LCD_RGB_ENABLE)
    #define OV5640_WORK_MODE CAM_MODE_LARGE_STD
    ...
    const uint16_t OV5640_INIT_Config[][2] =
        {
            ...
            {0x3037, 0x13}, // 分频
#if OV5640_WORK_MODE == CAM_MODE_SMALL_HIGH || OV5640_WORK_MODE == CAM_MODE_LARGE_HIGH
            {0x3036, 0x69}, // 超频一点点
#elif OV5640_WORK_MODE == CAM_MODE_LARGE_STD || OV5640_WORK_MODE == CAM_MODE_SMALL_STD
            {0x3036, 0x64}, // 倍频到标准800Mhz,后续再分频到80Mhz，一般认为是 STM32H750/743的DCMI接口所允许的最高速度
#endif
...
#if OV5640_WORK_MODE == CAM_MODE_SMALL_HIGH
            {0x3800, 0x00}, // HS，ISP窗口水平起始坐标
            {0x3801, 0x00}, // HS
            {0x3802, 0x00}, // VS，ISP窗口垂直起始坐标
            {0x3803, 0x04}, // VS
            {0x3804, 0x0a}, // HW (HE)，ISP窗口水平终点坐标
            {0x3805, 0x3f}, // HW (HE)
            {0x3806, 0x07}, // VH (VE)，ISP窗口垂直终点坐标
            {0x3807, 0x9b}, // VH (VE)
            // {0x3808, 0x01}, // 440 最后输出高字节
            // {0x3809, 0xB8}, // 440 低字节
            // {0x380a, 0x01}, // 330 最后输出高字节
            // {0x380b, 0x4A}, // 330 低字节
            {0x380e, 0x03}, // VTS
            {0x380f, 0xd5}, // VTS
            {0x380c, 0x05}, // HTS 
...
#elif OV5640_WORK_MODE == CAM_MODE_LARGE_HIGH
            {0x3800, 0x01}, // HS
            {0x3801, 0x50}, // HS
            {0x3802, 0x01}, // VS
            {0x3803, 0xb2}, // VS
            ...
            {0x3806, 0x05}, // VH (VE)
            {0x3807, 0xf1}, // VH (VE)
            // {0x3808, 0x03}, // 880 最后输出高字节
            // {0x3809, 0x70}, // 880 最后输出低字节
            // {0x380a, 0x01}, // 495 最后输出高字节
            // {0x380b, 0xEF}, // 495 最后输出低字节
            {0x380c, 0x08}, // HTS
            {0x380d, 0x80}, // HTS
            {0x380e, 0x04}, // VTS
            {0x380f, 0x60}, // VTS
...
#elif OV5640_WORK_MODE == CAM_MODE_SMALL_STD
            {0x3800, 0x00}, // HS，ISP窗口水平起始坐标
            {0x3801, 0x00}, // HS
            {0x3802, 0x00}, // VS，ISP窗口垂直起始坐标
            {0x3803, 0x04}, // VS
            ...
            {0x3814, 0x31}, // timing X inc
            {0x3815, 0x31}, // timing Y inc
#elif OV5640_WORK_MODE == CAM_MODE_LARGE_STD
            {0x3800,0x01}, // HS
            {0x3801,0x50}, // HS
            {0x3802,0x01}, // VS
            {0x3803,0xb2}, // VS
