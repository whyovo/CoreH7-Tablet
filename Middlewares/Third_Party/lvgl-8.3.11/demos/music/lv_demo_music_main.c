...
/* 引用外部的歌曲总数变量 */
extern uint32_t g_music_count;

/* 引用 music_task.c 中的控制函数 */
extern void Music_Set_Volume(uint8_t vol);
...
static uint32_t current_track_total_time = 0;
...
static void slider_event_cb(lv_event_t *e)
{
    ...
    time_act = (uint32_t)lv_slider_get_value(slider);

    // 更新时间显示
    lv_label_set_text_fmt(time_obj, "%" LV_PRIu32 ":%02" LV_PRIu32 " / %" LV_PRIu32 ":%02" LV_PRIu32,
                          time_act / 60, time_act % 60,
                          current_track_total_time / 60, current_track_total_time % 60);
}
static lv_obj_t *create_cont(lv_obj_t *parent)
{
...
static lv_obj_t *create_icon_box(lv_obj_t *parent)
{
    ...
    lv_obj_set_style_pad_gap(cont, 15, 0); // 控件间距

    // 1. 音量图标
    lv_obj_t *vol_icon = lv_label_create(cont);
    lv_label_set_text(vol_icon, LV_SYMBOL_VOLUME_MAX);
    lv_obj_set_style_text_color(vol_icon, lv_color_hex(0x504d6d), 0);
    lv_obj_set_style_text_font(vol_icon, font_large, 0);

    // 2. 音量滑条
    lv_obj_t *vol_slider = lv_slider_create(cont);
    lv_slider_set_range(vol_slider, 0, 100);
    lv_slider_set_value(vol_slider, 10, LV_ANIM_OFF); // 默认音量设置为 10
    lv_obj_set_width(vol_slider, 200);                // 宽度

    // 样式美化 (参考进度条样式)
    lv_obj_set_style_bg_color(vol_slider, lv_color_hex(0xd0d0d0), LV_PART_MAIN);
    lv_obj_set_style_bg_color(vol_slider, lv_color_hex(0x504d6d), LV_PART_INDICATOR);
    lv_obj_set_style_bg_color(vol_slider, lv_color_hex(0x504d6d), LV_PART_KNOB);
    lv_obj_set_style_pad_all(vol_slider, 2, LV_PART_KNOB);
...
static lv_obj_t *create_ctrl_box(lv_obj_t *parent)
{
    ...
    lv_label_set_text(time_obj, "0:00 / 0:00");
...
static void track_load(uint32_t id)
{
    // 防止传入非法的 id 导致 get_title 返回 NULL 进而崩溃
    if (id >= g_music_count)
        return;
    ...
    // 获取并缓存总时长，设置进度条范围
    current_track_total_time = _lv_demo_music_get_track_length(id);
    lv_slider_set_range(slider_obj, 0, current_track_total_time);
    lv_slider_set_value(slider_obj, 0, LV_ANIM_OFF);

    // 更新时间标签显示
    lv_label_set_text_fmt(time_obj, "0:00 / %" LV_PRIu32 ":%02" LV_PRIu32,
                          current_track_total_time / 60, current_track_total_time % 60);
    ...
    bool next = false;
    if ((track_id + 1) % g_music_count == id)
        next = true;
...
static lv_obj_t *album_img_create(lv_obj_t *parent)
{
    ...
    // 频谱数据根据 track_id 循环切换 (0,1,2,0,1,2...)
    // 这样即使有几十首歌，频谱动画也会有变化，不会越界
    switch (track_id % 3)
    {
    case 2:
        spectrum = spectrum_3;
...
static void timer_cb(lv_timer_t *t)
{
    ...
    // 防止当前时间超过总时间
    if (current_track_total_time > 0 && time_act > current_track_total_time)
    {
        time_act = current_track_total_time;
    }
