...
/* 引用 music_task.c 中的全局变量和函数 */
extern char *g_music_titles[];
extern uint32_t g_music_count;
extern uint32_t Music_Get_Duration(uint32_t index); // 【新增】
...
{
    // 从扫描到的全局列表中获取
    if (track_id >= g_music_count)
        return NULL;
    return g_music_titles[track_id];
}
...
const char *_lv_demo_music_get_artist(uint32_t track_id)
{
    if (track_id >= g_music_count)
        return NULL;
    return "Unknown Artist"; // 统一显示未知艺术家
}
...
{
    // 调用底层函数获取真实时长
    return Music_Get_Duration(track_id);
}
